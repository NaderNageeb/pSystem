...
class Rsdsale extends MY_Controller {
	public function __construct(){
		parent::__construct();
		$this->load_global();
		$this->load->model('rsdsale_model','rsdsale');
	}
	...
	public function index()
	{
		...
		$data=$this->data;
		$data['page_title']=$this->lang->line('rsd_Purchases');
		$this->load->view('rsdsale',$data);
	}
	public function add()
	{
...
public function add2()
	...
	{
		$list = $this->rsdsale->get_datatables();
		...
		foreach ($list as $customers) {
			...
			$row[] = $no++;
			$row[] = $customers->inv_no;
			$row[] = $customers->p_no;
			$row[] = $customers->p_name;
			$row[] = $customers->GTIN;
			$row[] = $customers->BN;
			$row[] = $customers->SN;
			$row[] = $customers->XD;
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->rsdsale->count_all(),
						"recordsFiltered" => $this->rsdsale->count_filtered(),
						"data" => $data,
				);
