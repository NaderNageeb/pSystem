...
class Rsderrorcode extends MY_Controller {
	public function __construct(){
		parent::__construct();
		$this->load_global();
		$this->load->model('Rsderrorcode_model','rsderrorcode');
	}
	...
	public function index()
	{
		...
		$data=$this->data;
		$data['page_title']=$this->lang->line('rsd_error_code');
		$this->load->view('rsderrorcode',$data);
	}
	public function add()
	{
...
public function add2()
	...
	{
		$list = $this->rsderrorcode->get_datatables();
		...
		foreach ($list as $customers) {
			...
			$row[] = $no++;
			$row[] = $customers->ErrorCode;
			$row[] = $customers->ArabicDesccription;
			$row[] = $customers->EngDescription;
			$data[] = $row;
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->rsderrorcode->count_all(),
						"recordsFiltered" => $this->rsderrorcode->count_filtered(),
						"data" => $data,
				);
