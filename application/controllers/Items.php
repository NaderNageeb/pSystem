	...
	public function addItemFromModal(){
		...
		if ($this->form_validation->run() == TRUE) {
			...
			$modal_post=array(
				                ...
				                'item_code' => $this->input->post('m_item_code'),
								...
								'custom_qrcode' => $this->input->post('m_custom_barcode'),
...
// 
if( $dateString1 >= $items->expire_date ){
	$row[] = '<span class="btn btn-danger" disabled>Expired</span>';	
}else{
	$row[] = $items->expire_date;
}
...
// returnsales :  for sales return
		...
		$display_json = array();
			...
			$itemCount = strlen($name);
			if($itemCount > 15){
               $part1 = substr($name, 0, 2); 
               $gtin = substr($name, 2, 14); 
               $shipment_number = substr($name, 18, 6); 
               $Expire_date = substr($name, 26, 6);
               $serial_number = substr($name, 40, 15);

			//    rsd salecancel table 
			   if(isset($search_for) &&  $search_for=='sales' && isset($returnsales) &&  $returnsales=='yes'){
				$salesreturnid	= get_count_id('db_salesreturn');
				
				$query = "insert into `salecancel`(`prodectname`,`ProductCod`,`GTIN`,`XD`,`statuss`,`SN`,`BN`,`isit`)values('$item_name','$item_id','$gtin','$Expire_date',0,'$serial_number','$shipment_number','$salesreturnid')";
			...
			//
// rsd saleservice table
			   if(isset($search_for) &&  $search_for=='sales' && !isset($returnsales)){

				

				$query = "insert into `saleservice`(`prodcode`,`prodect_qr`,`GTIN`,`PATIENTNATIONALID`,`XD`,`ststus`,`SN`,`BN`,`isit`)values('$item_name','$item_id','$gtin','$customer_id','$Expire_date',0,'$serial_number','$shipment_number','$count_id')";
				$query=$this->db->simple_query($query);
		        if(!$query){
			 return "failed";
		       }
			...
			//   
			// rsd accept_service for purchase table
			$purchase_countId = get_count_id('db_purchase');
			   if(isset($search_for) && $search_for=='purchase'){
			
				$query = "insert into `accept_service`(`p_name`,`p_no`,`GTIN`,`XD`,`statuss`,`SN`,`BN`,`isit`)values('$item_name','$item_id','$gtin','$Expire_date',0,'$serial_number','$shipment_number','$purchase_countId')";
				$query=$this->db->simple_query($query);
		        if(!$query){
