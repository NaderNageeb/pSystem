...
class Rsdsalescancel extends MY_Controller {
	public function __construct(){
		parent::__construct();
		$this->load_global();
		$this->load->model('Rsdsalescancel_model','rsdsalescancel');
	}
	...
	public function index()
	{
		...
		$data=$this->data;
		$data['page_title']=$this->lang->line('rsd_sale_cancel');
		$this->load->view('rsdsalescancel',$data);
	}
	public function add()
	{
...
public function add2()
	...
	{
		$list = $this->rsdsalescancel->get_datatables();
		...
		foreach ($list as $customers) {
			...
			$row[] = $no++;
			$row[] = $customers->ProductCod;
			$row[] = $customers->PRESCRIPTIONID;
			$row[] = $customers->prodectname;
			$row[] = $customers->TOGLN;
			$row[] = $customers->GTIN;
			$row[] = $customers->BN;
			$row[] = $customers->SN;
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->rsdsalescancel->count_all(),
						"recordsFiltered" => $this->rsdsalescancel->count_filtered(),
						"data" => $data,
				);
