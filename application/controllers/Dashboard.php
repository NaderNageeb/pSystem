...
class Dashboard extends MY_Controller {
	...
	public function expair_date_list(){
		// select query in between befor 5 days ago and next 7 days 
			//  $fivedays = date('Y-m-d', strtotime('-5 day'));
			 $today = date('Y-m-d');
			 $nextfivedays = date('Y-m-d', strtotime('+8 day'));
				...
				$this->db->where('expire_date <=', $nextfivedays);
		...
		if($q5->num_rows() >0){
			...
			foreach($q5->result() as $items){
				...
				$row = array();
							...
							if($deffdates==0 || $dateString1 > $items->expire_date ){
					...
					$output = array(
						 "draw" => $_POST['draw'],
						"recordsTotal" => $this->items->count_all(),
						 "recordsFiltered" => $this->items->count_filtered_expire(),
						"data" => $data,
					);
					 //output to json format
					echo json_encode($output);
