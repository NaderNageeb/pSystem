...
class Items_model extends CI_Model {
	...
	// Nader 27-11-2023
	function checkTheExpiredate($custom_barcode,$expire_date){
		$itemCount = strlen($custom_barcode);
		if($itemCount > 15){
		$newexpiredatepart = substr($custom_barcode,26, 6);
		return $newexpiredatepart;
		}else{
		return $expire_date;
		}
		}
	
			function checkTheNumOrcode($custom_barcode){
			$itemCount = strlen($custom_barcode);
						
			if($itemCount > 15){
						
			$newbarcodepart	= substr($custom_barcode, 2, 14);
						
			return $newbarcodepart;
			}else{
			return $custom_barcode;
				...
				$newbarcode = checkTheNumOrcode($custom_barcode);
				$newexpire_date = checkTheExpiredate($custom_barcode,$expire_date);
	// Nader 27-11-2023
