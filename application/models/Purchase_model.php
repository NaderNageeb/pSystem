...
class Purchase_model extends CI_Model {
	...
	public function verify_save_and_update(){
		...
		$gtin = GetitemGTIN($item_id);
		$sql = "select MAX(`count_id`) as count_id from db_purchase ";
		...
		for($i=1;$i<=$rowcount;$i++){
			...
			// rsd table accept_service  update

			
		$query = "UPDATE `accept_service` SET statuss = 1 , inv_no = '$reference_no' WHERE isit = '$num_rows' and p_no = $item_id ";
		...
		?>
            <tr id="row_<?=$rowcount;?>" data-row='<?=$rowcount;?>'>
               ...
               <input type="hidden" id="service_bit_<?=$rowcount;?>" name="service_bit_<?=$rowcount;?>" value="<?=$service_bit;?>">
			   	...
			   	<!--  -->
			<input type="hidden" id="item_expire_date_<?= $rowcount; ?>" name="item_expire_date_input_<?= $rowcount; ?>" value="<?= $expire_date; ?>">
			<input type="hidden" id="item_price_<?= $rowcount; ?>" name="item_price_input_<?= $rowcount; ?>" value="<?= $item_price; ?>">
			<!--  -->
	...
	public function GetitemGTIN($item_id){
