...
class Pos_model extends CI_Model {
	...
	public function get_details(){
		...
		$CI =& get_instance();
		  ...
		  if(!empty($item_name)){
			  ...
			  $this->db->where("(upper(a.item_name) like '%$item_name%' or upper(a.item_code) like '%$item_name%' or upper(a.custom_qrcode) like '%$item_name%' or upper(a.custom_barcode) like '%$item_name%')");
	      ...
	      if($q2->num_rows()>0){
	        foreach($q2->result() as $res2){
				...
				$item_expire_date = $res2->expire_date;
				...
				$EXPYEAR = substr($item_expire_date, 2, 3);
				$ex1 =  substr($item_expire_date, 5, 6);
				$short_cut_expire = "$EXPYEAR$ex1";
				$newshort_cut_expire = preg_replace("/[^a-zA-Z0-9]/","",$short_cut_expire);
	          ...
	          <div class="box box-default item_box" id="div_'.$res2->id.'" onclick="'.$str.'"
	          				...
	          				data-item-expire-date="'.$item_expire_date.'"
	...
	public function pos_save_update(){//Save or update sales
		...
		for($i=0;$i<$rowcount;$i++){
			...
			$query = "UPDATE `saleservice` SET 	ststus = 1 WHERE isit = '$count_id' and prodect_qr = '$item_id' ";
	    ...
	    if($q2->num_rows()>0){
		  ...
		  if($rows>0){
		  	...
		  	foreach ($q3->result() as $res3) { 
		  		...
		  		$item_expire_date = $res3->expire_date;
			    ...
			    $remove_btn      ='<a class="fa fa-fw fa-trash-o text-red" style="cursor: pointer;font-size: 20px;" onclick="removerow('.$i.')" title="Delete Item?"></a>';
		  		...
		  		echo '<td id="td_'.$i.'_7">'.$stock.'</td>';  /*td_0_1 item available qty*/
