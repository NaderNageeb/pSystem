			...
			<td align="center">
                ...
                <strong><?= $store_name; ?></strong><br>
		           ...
		           <?php //echo (!empty(trim($company_postcode))) ? "-".$company_postcode : '';?>
		            ...
		            <?php echo (!empty(trim($company_vat_number)) && vat_number()) ? $this->lang->line('')."الرقم الضريبي:".$company_vat_number."<br>" : '';?>
...
-->
                            ...
                            if(isset($sales_id)){
                              ...
                              if($q4->num_rows()>0){
                                ...
                                foreach ($q4->result() as $res4) {
                                  echo "<tr>";
                                  echo '<td style="border:1px solid;padding-center:2px;">'.$i++."</td>";
                                  echo '<td style="border:1px solid;padding-center:2px;">';
                                    echo $res4->payment_type;
                                  echo '</td>';
                                  echo '<td style="border:1px solid;text-align: center;padding-right:2px;">'.store_number_format($res4->payment).'</td>';
                                  echo "</tr>";
					...
					<tr>
						...
						<?php 
							$seller_name= $store_name; // هنا ينزل اسم المؤسسة الموجود في ترويسة الفاتورة

								$vat_registration_number= $company_vat_number; // هنا الرقم الضريبي برضو موجود في ترويسة الفاتورة

								$invoice_datetimez= $sales_date; // تاريخ الفاتورة ودائماً بكون تاريخ اليوم لكن عايزينه من الجدول برضو موجود في الفاتروة

								$invoice_amount= $grand_total; // هنا قيمة الفاتورة

								$invoice_tax_amount= $tax_amt; // هنا القيمة الضريبية في الفاتورة
								...
								// $sales_code = base64_encode($result);
								// echo $sales_code;
								$text = base64_encode($result);
								$text2 =base64_encode($text);
								 echo $CI->print_qr($text2);
