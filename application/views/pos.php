          ...
          <ul class="nav navbar-nav">
            ...
            <ul class="dropdown-menu">
              ...
              <li class="user-header">
                ...
                <p>
                 <?php print ($this->session->userdata('inv_username')); ?>
                  <small>سنة <?=date("Y");?></small>
                  <small class='text-uppercase text-bold'>الصلاحية: <?=$this->session->userdata('role_name');?></small>
                </p>
              </li>
              <!-- Menu Body -->
              <!-- Menu Footer-->
              <li class="user-footer">
                <div class="pull-left">
                  <a href="<?php echo $base_url; ?>users/edit/<?= $this->session->userdata('inv_userid'); ?>" class="btn btn-default btn-flat">بروفايل</a>
                </div>
                <div class="pull-right">
                  <a href="<?php echo $base_url; ?>logout" class="btn btn-default btn-flat">تسجيل الخروج</a>
                </div>
              </li>
            </ul>
          </li>
                ...
                <div class="input-group input-group-md">
                  ...
                  <input type="text" class="form-control" placeholder="Item name/Barcode/Itemcode" id="item_search">
                     <span class="input-group-addon pointer show_item_service" title="New Item?"><i class="fa fa-plus text-primary fa-lg"></i></span>
                      <!-- <input type="text" class="form-control" data-toggle="tooltip" title="Enter Item name/Barcode/Itemcode" placeholder="Item name/Barcode/Itemcode" id="item_name" name="item_name">
                      <span class="input-group-addon pointer show_item_service" title="New Item?"><i class="fa fa-plus text-primary fa-lg"></i></span> -->
                    ...
                    <div class="col-sm-12" style="overflow-y:auto;height: 300px;border:1px solid #337ab7;" >
                      <table class="table table-condensed table-bordered  table-responsive items_table" style="">
                        <thead class="bg-gray">
                          ...
                          <th width="30%"><?= $this->lang->line('expire_date'); ?></th>
...
function addrow(id='',item_obj=''){
    ...
    var item_expire_date = (item_obj=='') ? $('#div_'+id).attr('data-item-expire_date') : item_obj.expire_date; 
    ...
    var str=' <tr id="row_'+rowcount+'" data-row="0" data-item-id='+item_id+'>';/*item id*/
        str+='<td id="td_'+rowcount+'_0"><a data-toggle="tooltip" title="Click to Change Tax" class="pointer" id="td_data_'+rowcount+'_0" onclick="show_sales_item_modal('+rowcount+')">'+ item_name     +'</a> <i onclick="show_sales_item_modal('+rowcount+')" class="fa fa-edit pointer"></i></td>';/* td_0_0 item name*/ 
         // expire data
         str+='<td id="td_'+rowcount+'">'+  expire_date_compare(item_expire_date);   +'</td>';
        // expire date
 ...
 function expire_date_compare(item_expire_date){
  //  alert(item_expire_date);
 if (item_expire_date=="") {
  return 'No Date';
} else{
...
if (date2 >= item_expire_date) {
return '<span class="btn btn-danger"disabled>Expired</span>'; 
}else {
